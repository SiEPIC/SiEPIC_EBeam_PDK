...
class ring_singlebus(pya.PCellDeclarationHelper):
  ...
  def display_text_impl(self):
    ...
    return "ring_singlebus(R=" + ('%.3f' % self.r) + ",g=" + ('%g' % (1000*self.g)) + ",Lc=" + ('%g' % (1000*self.Lc)) + ",orthogonal_identifier=" + ('%s' % self.orthogonal_identifier) + ")"
  ...
  def produce_impl(self):
    ...
    wg_pcell = self.layout.create_cell("Waveguide", self.technology_name, {
                                                            "waveguide_type":self.waveguide_type,
                                                            "path": path
                                                                })
    if wg_pcell==None:
        raise Exception("problem! cannot create Waveguide from library: %s" % self.technology_name)
