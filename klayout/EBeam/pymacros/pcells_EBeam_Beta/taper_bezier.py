...
#    self.param("simulation", self.TypeCallback, "Launch simulation GUI")
    ...
    self.param("wg_width1", self.TypeDouble, "Waveguide Width1 (micron)", default = 0.5)
    self.param("wg_width2", self.TypeDouble, "Waveguide Width2 (micron)", default = 3)
    self.param("wg_length", self.TypeDouble, "Waveguide Length (micron)", default = 10)
  ...
  def display_text_impl(self):
    ...
    return "taper_bezier(" + ('%.3f-%.3f-%.3f-%.3f-%.3f' % (self.wg_width1,self.wg_width2,self.wg_length, self.a, self.b) ) + ")"
  ...
  def produce(self, layout, layers, parameters, cell):
    ...
    wg_Dpts =   bezier_cubic(pya.DPoint(0, self.wg_width1/2), pya.DPoint(self.wg_length, self.wg_width2/2), 0, 0, self.a, self.b, accuracy = self.accuracy) + \
                bezier_cubic(pya.DPoint(0, -self.wg_width1/2), pya.DPoint(self.wg_length, -self.wg_width2/2), 0, 0, self.a, self.b, accuracy = self.accuracy)[::-1]
    ...
    return "taper_bezier(" + ('%.3f-%.3f-%.3f' % (self.wg_width1,self.wg_width2,self.wg_length) ) + ")"
