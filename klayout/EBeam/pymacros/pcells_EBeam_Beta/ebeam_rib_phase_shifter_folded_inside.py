...
class ebeam_rib_phase_shifter_folded_inside(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("include_oxide_open", self.TypeBoolean, "Include thermal isolation etch, 0 - no; 1 - yes", default = 0)
    self.param("fold_number", self.TypeInt, "Number of folds (3, 5, 7, ...)", default = 5)   
    self.param("input_rib_width", self.TypeDouble, "Input Rib Width (um)", default = 0.35)
    self.param("input_slab_width", self.TypeDouble, "Input Slab Width (um)", default = 2)
    self.param("in_taper_length", self.TypeDouble, "Input Taper Length (um)", default = 15)    
...
#    self.param("ps_slab_width", self.TypeDouble, "Phase Shifter Slab Width (um)", default = 18)
    ...
    self.param("npp_distance", self.TypeDouble, "Npp Edge to Si Edge (um)", default = 2)
...
#    LayerNN = ly.layer(self.nlayer)
    ...
    if folding_n > 0:
      ...
      for i in range(0, folding_n):
        ...
        total_waveguide_length += layout_waveguide2(TECHNOLOGY, self.layout, self.cell, ['Si'], [folding_w*dbu], [0], pts, radius_um, adiab, bezier)
        pts = [Point(-l/2,- y_move), Point(l/2,- y_move)]
        total_waveguide_length += layout_waveguide2(TECHNOLOGY, self.layout, self.cell, ['Si'], [folding_w*dbu], [0], pts, radius_um, adiab, bezier)
