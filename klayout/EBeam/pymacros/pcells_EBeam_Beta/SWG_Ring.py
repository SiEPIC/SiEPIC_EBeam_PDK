...
class SWG_Ring(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
      ...
      self.param("r",      self.TypeDouble, "Radius [um]", default = 30) # minimum radius is 5 um in TE
    ...
    def produce_impl(self):
      ...
      if busL <= 10:
        busL = taperL*2+10
        if debug:
            print('invalid length of SWG bus waveguide, set length at least 2 times larger than taper length and plus 10 um')
      ...
      nSeg = int(math.floor(angle/(math.degrees(theta1)+math.degrees(theta2)))) # NUmber of SWG segments
      ...
      xo = [(r+w/2+gap2),(r+w/2+gap2+w),(r+w/2+gap2+w),(r+w/2+gap2)]
      yo = [0,0,s1,s1]
      dpts=[pya.DPoint(xo[i], yo[i]) for i in range(len(xo))]
      dpolygon = DPolygon(dpts)
      element = Polygon.from_dpoly(dpolygon*(1.0/dbu))
      ...
      xtu = [((r+w/2+gap2)+(w-0.06)/2),((r+w/2+gap2)+(w-0.06)/2+0.06),((r+w/2+gap2)+w),((r+w/2+gap2))]
