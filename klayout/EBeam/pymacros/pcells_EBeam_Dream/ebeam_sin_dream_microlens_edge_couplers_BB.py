...
class ebeam_sin_dream_microlens_edge_couplers_BB(pya.PCellDeclarationHelper):
    """
    The PCell declaration for the ebeam_sin_dream_microlens_edge_couplers_BB
    ...
    def __init__(self):
        ...
        super(ebeam_sin_dream_microlens_edge_couplers_BB, self).__init__()
        ...
        self.param(
            "ref_wg", self.TypeBoolean, "Include reference waveguide", default=True
        )

        # declare the layers
        self.param(
            "sinlayer", self.TypeLayer, "SiN Layer", default=TECHNOLOGY["SiN"], hidden=True
        )
        self.param(
    ...
    def produce(self, layout, layers, parameters, cell):
        ...
        LayerSiN = ly.layer(self.sinlayer)
        ...
        if wavelength == 1310:
            ...
            w_waveguide = to_itype(0.850, dbu)
        elif wavelength == 1550:
            waveguide_type = "SiN Strip TE 1550 nm, w=800 nm"
            w_waveguide = to_itype(0.800, dbu)
        ...
        if self.ref_wg:
            ...
            loopback_path = pya.DPath(
                [
                    pya.DPoint(0, -127),
                    pya.DPoint((offset + l_taper + Lw2) * dbu, -127),
                    pya.DPoint((offset + l_taper + Lw2) * dbu, -254),
                    pya.DPoint(0, -254),
                ],
                0.8,
            )
        ...
        for n_ch in range(int(num_channels)):
            ...
            # draw the taper
            taper_pts = [
                pya.Point(0, -w_waveguide / 2 + pitch * n_ch),
                pya.Point(0, w_waveguide / 2 + pitch * n_ch),
                pya.Point(offset + l_taper + Lw3, w_waveguide / 2 + pitch * n_ch),
                pya.Point(offset + l_taper + Lw3, -w_waveguide / 2 + pitch * n_ch),
            ]
            ...
            pin = pya.Path(
                [pya.Point(-pin_length / 2, 0), pya.Point(pin_length / 2, 0)],
                w_waveguide,
            )
        ...
        if self.ref_wg:
            devrec_pts = [
                pya.Point(0, pitch * n_ch + 30 / dbu),
                pya.Point(x, pitch * n_ch + 30 / dbu),
                pya.Point(x, -pitch * 2 - 30 / dbu),
                pya.Point(0, -pitch * 2 - 30 / dbu),
            ]
        else:
            devrec_pts = [
                pya.Point(0, pitch * n_ch + 30 / dbu),
        ...
        if self.ref_wg:
            for n_ch in range(int(num_channels + 2)):
                lens_pts = [
                    pya.Point(0, -width_lens / 2 + pitch * n_ch - 2 * pitch),
                    pya.Point(0, width_lens / 2 + pitch * n_ch - 2 * pitch),
                    pya.Point(-length_lens, width_lens / 2 + pitch * n_ch - 2 * pitch),
                    pya.Point(-length_lens, -width_lens / 2 + pitch * n_ch - 2 * pitch),
                ]
            ...
            for n_ch in range(int(num_channels)):
                lens_pts = [
                    pya.Point(0, -width_lens / 2 + pitch * n_ch),
                    pya.Point(0, width_lens / 2 + pitch * n_ch),
                    pya.Point(-length_lens, width_lens / 2 + pitch * n_ch),
                    pya.Point(-length_lens, -width_lens / 2 + pitch * n_ch),
                ]
                shapes(LayerBBN).insert(pya.Polygon(lens_pts))
