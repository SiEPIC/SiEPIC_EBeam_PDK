...
class ebeam_dream_microlens_edge_couplers_BB(pya.PCellDeclarationHelper):
    ...
    def __init__(self):
        ...
        self.param("waveguidelayer",self.TypeLayer, "Waveguide Layer", default = TECHNOLOGY['Waveguide'])
    ...
    def produce(self, layout, layers, parameters, cell):
        ...
        loopback_path = DPath([DPoint(0,-127),DPoint((offset + l_taper + Lw2)*dbu+15,-127),DPoint((offset + l_taper + Lw2)*dbu+15,-254),DPoint(0,-254)],0.5)
        ...
        for n_ch in range(int(num_channels)):
            ...
            #draw the taper
            taper_pts = [Point(0,-w_waveguide/2+pitch*n_ch),Point(0,w_waveguide/2+pitch*n_ch),Point(offset + l_taper + Lw3,w_waveguide/2+pitch*n_ch),Point(offset + l_taper + Lw3,-w_waveguide/2+pitch*n_ch)]
            ...
            pin = Path([Point(-pin_length/2,0),Point(pin_length/2,0)], w_waveguide)
            pin_t = pin.transformed(t)
            shapes(LayerPinRecN).insert(pin_t)
            text = Text(f"opt{n_ch+1}",t)
        ...
        #draw devrec box
        devrec_pts = [Point(0,pitch*n_ch+30/dbu),Point(x,pitch*n_ch+30/dbu),Point(x,-pitch*2-30/dbu),Point(0,-pitch*2-30/dbu)]
        ...
        text = Text("  Number of Channel(s): " + str(num_channels) + "\n  Center Wavelength: " + str(wavelength) + " nm",t)
        shape = shapes(LayerTEXTN).insert(text)
        ...
        for n_ch in range(int(num_channels+2)):
            #draw the taper
            taper_pts = [Point(0,-width_lens/2+pitch*n_ch-2*pitch), Point(0,width_lens/2+pitch*n_ch-2*pitch), Point(-length_lens,width_lens/2+pitch*n_ch-2*pitch),Point(-length_lens,-width_lens/2+pitch*n_ch-2*pitch)]

            #place the taper
            shapes(LayerBBN).insert(Polygon(taper_pts))
   
            #draw fibre target circle
            align_circle = circle(-length_lens,pitch*n_ch-2*pitch,25/dbu)

            #place fibre target circle
            shapes(LayerBBN).insert(Polygon(align_circle))
