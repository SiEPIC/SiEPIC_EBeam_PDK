...
class TestStruct_DoubleBus_Ring2(pya.PCellDeclarationHelper):
    ...
    def produce_impl(self):
        ...
        instance = cell.insert(
            CellInstArray(GC_imported, t, Point(0, 127 / dbu), Point(0, 0), 4, 1)
        )
        print(instance.cell_index)
        ...
        points = [
            [0, 0],
            [10 + 2 * r + 2 * g + 2 * wg_width, 0],
            [10 + 2 * r + 2 * g + 2 * wg_width, y_ring - 2 * r - wg_width],
        ]
        ...
        points = [
            [0, 127 * 3],
            [10 + 2 * r + 2 * g + 2 * wg_width, 127 * 3],
            [10 + 2 * r + 2 * g + 2 * wg_width, y_ring],
        ]
        layout_waveguide_abs(cell, LayerSi, points, wg_width, 20)

        print(
            "Done drawing the layout for - TestStruct_DoubleBus_Ring2: %.3f-%g" % (r, g)
        )
