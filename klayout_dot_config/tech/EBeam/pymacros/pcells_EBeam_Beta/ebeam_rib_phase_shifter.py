...
class ebeam_rib_phase_shifter(pya.PCellDeclarationHelper):
  ...
  def __init__(self):
    ...
    self.param("npp_distance", self.TypeDouble, "Npp Edge to Si Waveguide (um)", default = 2)
...
#    self.param("vc_dw", self.TypeDouble, "VC Offset", default = 5)
    ...
    self.param("vc_si_distance", self.TypeDouble, "VC Edge to Si Waveguide (um)", default = 3)
...
#    LayerNN = ly.layer(self.nlayer)
    ...
    if 1:
      ...
      for i in range(0,N + 1):
        pts.append(Point(-l/2 - in_taper + in_taper/N * (N - i), - (in_slab-overlay_ebl*2)/2 - ((sw/2  - (in_slab-overlay_ebl*2))/(N**order))*((N-i)**order)))
      ...
      for i in range(0,N + 1):
        pts.append(Point(l/2 + in_taper - in_taper/N * i, (in_slab-overlay_ebl*2)/2 + ((sw/2  - (in_slab-overlay_ebl*2))/(N**order))*(i**order)))
      for i in range(0,N + 1):
        pts.append(Point(l/2 + in_taper/N * i, - (in_slab-overlay_ebl*2)/2 - ((sw/2  - (in_slab-overlay_ebl*2))/(N**order))*((N-i)**order)))
      self.cell.shapes(LayerSlab).insert(Polygon(pts))
